...
import time
from LoopSolver import Elasticity
...
if __name__ == "__main__":
    ...
    implicit_tariff = PSO_Parameters['implicit_tariff'][0] #Tariff assumed when 8760 ran
    price_elasticity = PSO_Parameters['price_elasticity'][0] #Price elasticity for tariff
    ...
    while iteration < maxGen-1 and t_diff > stopLimit and match < convergenceRequirement:
        ...
        for m in range(numInd):            
            ...
            Propane_ec[m,iteration], Batt_SOC, LoadkW, P_gen, P_PV, P_batt, P_dump,Limit_charge, Limit_discharge, BattkW, Batt_kWh_tot_ec[m,iteration],loadkWh,peakload, lifecycle = Tech_total(Parameters[0,m,iteration],Parameters[1,m,iteration],0) #calculate baseline tech tools values to feed parameters into Elasticity
            #don't need to save Gt_panel, final, Batt_SOC, and Charge these are used to validate program
               
            #LoanPrincipal, year, Cost, Revenue, CashonHand, Balance, M, O, tariff[m,iteration], Batt_life_yrs[m,iteration], Coeff = Econ_total(Propane_ec[m,iteration],Parameters[1,m,iteration]*peakload,Parameters[0,m,iteration]*peakload,Batt_kWh_tot_ec[m,iteration],peakload,loadkWh, lifecycle)
            #order of parameters: batt, PV, CSP, ORC, TES_ratio
            Propane_ec[m,iteration], Batt_SOC, LoadkW, P_gen, P_PV, P_batt, P_dump,Limit_charge, Limit_discharge, BattkW, Batt_kWh_tot_ec[m,iteration],loadkWh,peakload, lifecycle, LoanPrincipal, year, Cost, Revenue, CashonHand, Balance, M, O, tariff[m,iteration], Batt_life_yrs[m,iteration] = Elasticity(implicit_tariff, price_elasticity, Parameters[0,m,iteration],Parameters[1,m,iteration])
    ...
    gB_total_var =pd.DataFrame({'Total Cost':[Total_Cost],'Simulation_Time':[total_time], 'Peakload':[peakload]})
