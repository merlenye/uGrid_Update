...
## Generation Control =========================================================
def GenControl(P_PV,L,Batt_discharge, Batt_charge,freespace_discharge,genPeak,LoadLeft,Batt_SOC,dayhour, BattkWh, trans_losses):
...
# solar PV generation. 
    ...
    if P_PV > 0:
        ...
        if freespace_discharge > LoadLeft and Batt_discharge > L: #enough charge to run through night and meet load demand, PV and gen off
            ...
            P_batt = -(P_gen-L)
            P_dump = 0
    if P_batt > 0: #If and else statements account for transmission loss
        #If P_batt is positive, and the battery is loosing energy, trans loss accounts for extra energy needed to account for losses
        Batt_SOC = Batt_SOC - ((1+trans_losses)*(P_batt)) #update state of charge of the battery
        #1/.9 coefficient is to account for the 10% more energy required due to loss in transmission
    else:
        Batt_SOC = Batt_SOC - ((1-trans_losses)*P_batt) #If P_batt is negative, and battery is charging, 1- transmission loss is to account for ~10% (UI) energy lost to heat during transmission
      
    #Batt_SOC = Batt_SOC - P_batt
    if Batt_SOC > BattkWh: #Ensures that battery is not charged past its physical limit
        Batt_SOC = BattkWh
...
#"generator fuel consumption csalculations based on efficiency as a function of power output fraction of nameplate rating"
...
## lifecycle FUNCTION =====================================================================================================
def lifecycle(Batt_min):
    #This functions cycles through the depth of discharge at every hour to determine the maximum depth of discharge for each day
    #The output is the yearly sum of cycles for each day pertaining to the conditions at the time of maximum depth of discharge
    day = 0
    cycles = np.zeros(len(Batt_min))
    Lifecycle = 0
    Dod_max =0 
    T_batt =0
    avg = 0
    ...
    tot_avg=0
    temp_avg = 0
    for i in range (len(Batt_min)): #Loop through the depth of discharge dataset built in the operation function
        tot_avg += Batt_min[i,1]
        temp_avg += Batt_min[i,2]
        if Batt_min[i,1] > Dod_max:
            #Check if new Dod value exceeds the previous maximum for that day
            Dod_max = Batt_min[i,1]
            #Update daily maximum
            T_batt = Batt_min[i,2]
            #Update temperature at daily maximum depth of discharge
            
        
        if Batt_min[i,0] == 23 or i == (len(Batt_min)-1): #Conditional for the end of the day or the last value in the data frame
            
            #Account for the ceiling and floor temperature for the HVAC system to kick in
            #future updates could optimize these sparameters against HVAC load
            if T_batt > 30:
                T_batt = 30
            elif T_batt < 10:
                T_batt = 10
            ...
            #Calculate the battery cycles given the daily maximum conditions
            #Curve fitting algorithim used and R^2 = 0.99052569
            #https://www.systems-sunlight.com/wp-content/uploads/2018/11/EN-SUNLIGHT-Reserve-Power-Batteries-RES-OPzV-Sales-Web.pdf
            #if Dod_max < 1 or T_batt ==0:
                #((5808.64303086507 -88337.9757427706/T_batt -27.2532414505341 + 
                  #555544.874053075 - 82.1187653097685*T_batt)/365)
            #print(Dod_max)  
            #print(T_batt)
            cycles[day] = ((5808.64303086507 -88337.9757427706/T_batt -27.2532414505341/Dod_max**2 + 
                  555544.874053075/(Dod_max*T_batt**2) - 82.1187653097685*T_batt)/365)
            #Update the sum of each day in a year
            avg += Dod_max
            avgtemp += T_batt
            Lifecycle += cycles[day]
            #print("day cycle" + str(cycles[day]))
            #print("DoD max" + str(Dod_max))
            day +=1
    ...
    #print("avg" + str(avg/365))
    #print("avg temp" + str(avgtemp/365))
    #print("tot_avg" + str((tot_avg/(24*365))))
    #print("temp_avg" +str((temp_avg/(24*365))))
...
# the year by calling all of the previous functions and the solar python file.    
    ...
    while h < hmax-16:
        ...
        #Above was multiplied by Trans Losses, but that was removed
        ...
        if T_amb < 10:
            T_amb = 10
        T_batt = T_amb+(21.08851864412*DoD + 44.5211346966066*Batt_Charge_Limit*DoD - 2.214216042187 - 26.041327275384*DoD*(Batt_Charge_Limit**2))
        #print("temp is " + str(T_batt))
        Batt_discharge, Batt_charge, freespace_discharge = batt_calcs(timestep,BattkWh,T_batt,Batt_SOC[h],Limit_charge,Limit_discharge)
        P_batt[h], P_gen[h], Batt_SOC[h+1], P_dump[h] = GenControl(P_PV[h],LoadkW[h],Batt_discharge, Batt_charge,freespace_discharge,peakload,loadLeft[h],Batt_SOC[h],dayHour[h], BattkWh, trans_losses)
        Batt_min[h,0] = dayHour[h] #record the timestep for the array used in lifecycle function
        #print("time" + str(Batt_min[h,0]))
        #print("batt" + str(Batt_SOC[h]))
        #print("total" + str(BattkWh))
        Batt_min[h,1] = (1-(Batt_SOC[h]/BattkWh)) #record the battery state of charge for the array used in the lifecycle function. State of charge here is current charge of battery divided by full capacity
        #print("Batt_min" + str(Batt_min[h,1]))
        #print("SOC Fraction " + str(1-(Batt_SOC[h]/BattkWh)))
        #print("discharge" + str(Batt_min[h,1]))
        Batt_min[h,2] = T_batt
        #print("temp"+ str(Batt_min[h,2]))
    ...
    return Propane, Batt_SOC, LoadkW, P_gen, P_PV, P_batt, P_dump, Limit_charge, Limit_discharge, Batt_kWh_tot, Lifecycle
...
## Tech_total function =========================================================================================================
def Tech_total(BattkWh_Parametric,PVkW_Parametric, modified):
# This function calls the previous functions, excluding the plotting function, to solve for the power flows for the year.
    ...
    if modified == 0:
        ...
        LoadKW_MAK = pd.read_excel('LoadKW_MAK.xlsx',index_col=None, header=None)
        LoadKW_MAK= LoadKW_MAK * modified
    ...
    if modified == 0:
        loadkWh = sum(LoadKW_MAK[0])/12 #Keep original 8760 value for baseline run
    else:
        loadkWh = (sum(LoadKW_MAK[0])/12)*modified #adjust 8760 based on coeffecient when called from the solver module

    PVkW=PVkW_Parametric*peakload  #"[kW]"
    
    Propane, Batt_SOC, LoadkW, P_gen, P_PV, P_batt, P_dump,Limit_charge, Limit_discharge, Batt_kWh_tot, Lifecycle = operation(Tech_Parameters['Batt_Charge_Limit'][0], Tech_Parameters['smart'][0], PVkW, BattkWh, peakload, LoadKW_MAK, FullYearEnergy, MSU_TMY,Solar_Parameters,Tech_Parameters['trans_losses'][0]) 
    
    return Propane, Batt_SOC, LoadkW, P_gen, P_PV, P_batt, P_dump, Limit_charge, Limit_discharge,BattkWh,Batt_kWh_tot,loadkWh,peakload, Lifecycle
##=============================================================================================================
...
# PVkW_Parametric need to be specified. This is also where the plotting functions can be called, and t1 and t2 are specified. 
    ...
    Propane, Batt_SOC, LoadkW, P_gen, P_PV, P_batt, P_dump,Limit_charge, Limit_discharge, BattkW, Batt_kWh_tot,loadkWh,peakload, Lifecycle = Tech_total(BattkWh_Parametric,PVkW_Parametric)
